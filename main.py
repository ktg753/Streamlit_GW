...
if support_type == "CAT Tool":
    ...
    cat_issue = st.radio(
        "CAT Tool 종류",
        ["Trados", "Phrase", "MemoQ", "Passolo", "Xbench", "Across/Catalyst"]
    )

    if cat_issue == "Trados":
        trados_issue = st.radio(
            "Trados 이슈",
            ["라이선스", "프로젝트 생성/설정", "Add-in/Plug-in", "태그 제작", "기타"]
        )

        if trados_issue == "라이선스":
            license_issue = st.radio(
                "라이선스",
                ["No license type found 오류", "Evaluation Expired [37] 오류"]
            )
        ...
        if trados_issue == "프로젝트 생성/설정":
            project_issue = st.radio(
                "프로젝트 생성/설정",
                ["System.OutOfMemoryException 오류 발생", "리턴 패키지 XLIFF 유효성 오류 발생"]
            )
